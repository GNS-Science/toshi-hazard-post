...
def run_aggregation(args: AggregationArgs, pool_executor: Executor | None = None) -> None:
    ...
    num_jobs = 0
    log.info(f"starting {len(sites) * len(imts)} calculations with {NUM_WORKERS} workers")
    ...
    log.info(f"total time: processed {num_jobs} calculations in {time1 - time0:0.3f} seconds")
    log.info(f"time to perform aggregations after job setup {time_parallel_end - time_parallel_start:0.3f}")
